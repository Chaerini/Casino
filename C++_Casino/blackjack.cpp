...
inline char getSuitSymbol(char suit) {
    switch (suit) {
    case 'S': return 'S'; // Spades
    case 'C': return 'C'; // Clubs
    case 'H': return 'H'; // Hearts
    case 'D': return 'D'; // Diamonds
...
inline void printCardsHorizontally(const vector<Card>& cards) {
    ...
    for (const Card& card : cards) {
        ...
        midLine2.push_back("|       |");
        midLine3.push_back("| " + string(5 - value.length(), ' ') + value + " |");
        midLine4.push_back("|       |");
