...
void displayYellowText(const string& text) {
    cout << "\033[1;33m" << text << "\033[0m";  // 노란색
}

void displayRedText(const string& text) {
    cout << "\033[1;31m" << text << "\033[0m";  // 빨간색
}

void displayPurpleText(const string& text) {
    cout << "\033[1;35m" << text << "\033[0m";  // 보라색
}

void displayBlueText(const string& text) {
    cout << "\033[1;34m" << text << "\033[0m";  // 파란색
}

// 단어에 따라 색상 출력 함수
void printWithColor(const string& text) {
    if (text == "실탄") {
        displayRedText(text);
    }
    else if (text.find("체력") != string::npos) {
        displayPurpleText(text);
    }
    else if (text == "아이템" || text == "톱" || text == "돋보기" || text == "수갑" || text == "맥주캔") {
        displayBlueText(text);
    }
    else {
        cout << text;
    }
...
// 문장에 특정 단어들에 색상을 적용하는 함수
void printSentenceWithColor(const string& sentence) {
    string word;
    for (char c : sentence) {
        if (isspace(c)) {
            printWithColor(word);
...
void displayGameDescription() {
    ...
    cout << " 설명----\n";
    printWithColor("돋보기");
    cout << ": 현재 산탄총에 장전된 탄환을 확인할 수 있습니다.\n";
    printWithColor("맥주캔");
    cout << ": 현재 장전된 탄환을 안전하게 배출하여 턴을 넘길 수 있습니다.\n";
    printWithColor("톱");
    cout << ": 실탄이 장전될 경우 두 배의 데미지를 입힙니다.\n";
    printWithColor("수갑");
    cout << ": 상대방의 다음 턴을 건너뛰게 만듭니다.\n\n";
}
...
public:
    ...
    bool shoot(Player* opponent, vector<string>& bullets, bool nextShotDoubleDamage, bool isDealer = false) {
        ...
        if (target == 1) {
            cout << name << "가 자신에게 ";
            printWithColor(bullet);
            cout << "을(를) 쏘았습니다! ";
            printWithColor("체력");
            cout << "이 " << damage << " 감소합니다." << endl;
            health -= damage;
        }
        else {
            cout << name << "가 " << opponent->name << "에게 ";
            printWithColor(bullet);
            cout << "을(를) 쏘았습니다! 상대방의 ";
            printWithColor("체력");
            cout << "이 " << damage << " 감소합니다." << endl;
    ...
    bool useItem(const string& item, Player* opponent, vector<string>& bullets, bool& skipOpponentTurn, bool& nextShotDoubleDamage) {
        ...
        if (item == "돋보기") {
            if (!bullets.empty()) {
                cout << name << "가 돋보기를 사용하여 첫 번째 탄환을 확인했습니다: ";
                printWithColor(bullets.back());
                cout << endl;
            }
            else {
            ...
            if (!bullets.empty()) {
                ...
                cout << name << "가 맥주캔을 사용하여 안전하게 탄환을 배출했습니다. 배출된 탄환: ";
                printWithColor(discardedBullet);
                cout << endl;
                return true;
            }
        }
...
public:
    ...
    bool dealerTurn() {
        ...
        while (!dealer.items.empty()) {   // items는 dealer 객체의 멤버 변수임
            ...
            // 딜러가 이미 사용한 아이템이라면 건너뛰지 않고 아이템을 제거
            if (usedItems.count(item)) {
                dealer.items.erase(dealer.items.begin() + randomIdx);  // 사용된 아이템을 제거
                continue;
            }
    ...
    bool playerTurn() {
        ...
        // bullets가 비어있는지 확인
        if (bullets.empty()) {
            cout << "모든 탄환이 소진되었습니다. 플레이어의 턴이 종료됩니다.\n";
            return true;  // 턴 종료
        }
    ...
    void startGame() {
        ...
        if (dealer.health <= 0) {
            ...
            cout << "베팅 금액의 3배를 획득했습니다:" << betAmount*3 << endl;
        }
        else if (player.health <= 0) {
            cout << dealer.name << "가 승리했습니다!" << endl;
            availableAmount -= betAmount;
            cout << "베팅 금액을 잃었습니다: " << betAmount << endl;
        }
    }
};
